...
app.get('/', (req, res) => {   // to handle the get request
  ...
  if (query) {
    ...
    if (result !== null) {
      const formattedExpression = formatExpression(query);  // to format expression in desired json format
      ...
      res.json({ question: formattedExpression, answer: result }); // to give the response of the request
    } else {
...
function formatExpression(expression) {  // changing the format of expression which needs to be displayed according to requirement
